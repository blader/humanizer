...
async function getIssues(repo, state = 'open') {
  ...
  return issues
    ...
    .map(issue => ({
      ...
      is_enhancement: issue.labels.includes('enhancement') || issue.labels.includes('💡 Enhancement'),
...
async function getRepoMetadata(repo) {
  ...
  return {
    ...
    has_vulnerability_alerts: repoData.security_and_analysis?.dependabot_security_updates?.status === 'enabled',
...
function analyzePRs(prs) {
  const analysis = {
    ...
    by_category: {
      dependency_updates: prs.filter(pr => pr.title.includes('deps') || pr.title.includes('bump')).length,
      features: prs.filter(pr => pr.labels.includes('feature') || pr.title.startsWith('feat:')).length,
      bug_fixes: prs.filter(pr => pr.labels.includes('bug') || pr.title.startsWith('fix:')).length,
      documentation: prs.filter(pr => pr.labels.includes('documentation') || pr.title.startsWith('docs:')).length,
      other: 0
    }
  };
  
  analysis.by_category.other = analysis.total - 
...
async function main() {
  ...
  try {
    ...
    console.log('=== Summary ===\n');
    console.log(`Local Repository (${LOCAL_REPO}):`);
    console.log(`  - Open PRs: ${localPRAnalysis.total} (${localPRAnalysis.dependabot} Dependabot, ${localPRAnalysis.human_authored} human)`);
    console.log(`  - Open Issues: ${localIssueAnalysis.total} (${localIssueAnalysis.by_type.bugs} bugs, ${localIssueAnalysis.by_type.enhancements} enhancements)`);
    console.log(`  - Security Policy: ${localMetadata.has_security_policy ? 'Yes' : 'No'}`);
    
    console.log(`\nUpstream Repository (${UPSTREAM_REPO}):`);
    console.log(`  - Open PRs: ${upstreamPRAnalysis.total} (${upstreamPRAnalysis.dependabot} Dependabot, ${upstreamPRAnalysis.human_authored} human)`);
    console.log(`  - Open Issues: ${upstreamIssueAnalysis.total} (${upstreamIssueAnalysis.by_type.bugs} bugs, ${upstreamIssueAnalysis.by_type.enhancements} enhancements)`);
